...
- Container-oriented deployment formalised: every service has its
  own Dockerfile, the umbrella `dev-runner` orchestrates the full
  topology with `docker-compose`, and the production target is an
  **Oracle Cloud Always-Free A1.Flex VM (4 OCPU / 24 GB)** fronted by
  **Cloudflare Pages** (FrontEnd) + **Cloudflare Tunnel** (backend
  ingress, no inbound port forward).
...
| `email-send` — Email Service *(new)* | Java 21 + Spring Boot 3.3 + Spring Data MongoDB + sendgrid-java + jjwt | Outbound transactional email via SendGrid (sandbox-mode by default in dev). Persists every attempt to `email-mongo` (`email_events` collection). |
