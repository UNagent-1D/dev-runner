...
**Prototype 2 functional scope (delta vs P1):**

- Web front-end migrated from Vite SPA to **Next.js with Server-Side
  Rendering** to satisfy the "presentation must follow SSR" rule.
- Hospital domain backed by a real **PostgreSQL** instance (the
  `hospital-postgres` container) with idempotent migrations + seeds
  applied via `/docker-entrypoint-initdb.d`.
- New **Compliance** service (Python/FastAPI) replacing Metricas; same
  endpoints + the new `/v1/event` and `/v1/feedback` audit endpoints
  whose payloads are persisted to MongoDB.
- New **Email** service (Java 21 / Spring Boot 3.3) introducing the
  fifth general-purpose language and a fourth + fifth data store
  (local `email-mongo` Mongo + audit collection).
- New **RabbitMQ** broker (`UN_message_broker_mb`) wired into both
...
#### Topology
  ...
  - Edge tier — Cloudflare global PoPs:
    - Cloudflare Pages serves the FrontEnd (Vite SPA build) at https://app.unagent.site. The VITE_*
  API URLs are baked into the bundle at build time so the static asset edge-caches without runtime
  configuration.
    - Cloudflare Tunnel (cloudflared as a systemd unit on the host) exposes the backend HTTP services
  through stable HTTPS sub-domains: api, orch, chat, metrics, and email.unagent.site. The tunnel opens
   only outbound TCP from the host to the Cloudflare edge — no inbound port forwarding, no public IP,
  and no TLS certificate management on the host.
  - Application tier — a single Linux host running Docker Engine 29 + containerd 2.2.3 + runc 1.4. All
   twelve service containers live on the archsoft Docker bridge network started by a single docker
  compose up -d --build invocation:
    - infrastructure: redis, rabbitmq, hospital-postgres, email-mongo
    - logic: tenant, hospital-mock, agent-runtime, conversation-chat, chat-orch, compliance,
  email-send
  ...
  - External SaaS (free tiers):
    ...
    - MongoDB Atlas for both conversation-chat sessions and the Compliance audit log
    - OpenRouter as the LLM gateway, currently pinned to openai/gpt-4o-mini for reliability
  ...
  The unagent.site domain is registered with Namecheap and delegated to Cloudflare nameservers. All
  sub-domains (app, api, orch, chat, metrics, email, plus the SendGrid DKIM CNAMEs em, s1._domainkey,
  s2._domainkey for outbound deliverability) are managed in Cloudflare DNS. App and API records are
  proxied through Cloudflare; SendGrid DKIM records are DNS-only because Cloudflare's proxy munges
  CNAMEs and breaks DKIM signing.

  What was tested and rejected

  - Oracle Cloud Always-Free A1.Flex (4 OCPU / 24 GB ARM) was the original target. The provisioning
  script (scripts/oci-bootstrap.sh, OCI CLI–driven, idempotent) ran successfully; the Bogotá region
  exhausted A1 capacity for over 25 retry attempts and São Paulo blocked the larger shape under
  Free-Trial quota.
  - DigitalOcean Free Tier required a $10 verification hold that was outside the team's budget for the
   prototype.

  The Cloudflare Tunnel + local-host approach was chosen because it satisfies the full demo end-to-end
   with zero infrastructure cost and zero public-network exposure of the developer machine.

  #### Verification

  The deployment was validated end-to-end across all four delivery surfaces:

  1. REST + SSE chat — POST https://orch.unagent.site/v1/chat returns a streamed Spanish reply within
  ~5 s; list_doctors, get_doctor_schedule, book_appointment and the new send_confirmation_email tool
  calls all execute against hospital-mock and email-send respectively, with audit rows persisted to
  MongoDB Atlas (Compliance) and the local email-mongo container (email-send).
  2. Telegram channel — @UNAgent777_bot long-polls getUpdates from chat-orch; full multi-turn
  appointment-booking flow including emergency-detection escalation behaves correctly.
  3. Email confirmation — after book_appointment succeeds, the LLM autonomously emits
  send_confirmation_email; SendGrid responds 202 with the appointment ID as idempotency key; the email
   is delivered to the recipient inbox using the verified sender UN_Agent <nizulu1998@gmail.com>.
  4. Web SSR FrontEnd — https://app.unagent.site returns HTTP 200 with the bundle's API URLs pointing
  at the tunnelled backend hostnames; CORS is gated to that exact origin via the CORS_ALLOW_ORIGIN
  environment variable on chat-orch.

  #### Operational notes

  - The host must remain awake and connected during the demo; cloudflared and docker survive
  lock-screen but not suspend. Sleep and lid-close suspend are inhibited for the duration of the
  presentation via systemd-inhibit --what=sleep:idle:handle-lid-switch.
  - Container state is persistent across reboots through two named volumes: hospital-postgres-data and
   email-mongo-data. Atlas, Supabase and SendGrid keep their state independently.
  - All credentials (OpenRouter, JWT secret, Supabase URI, Atlas URIs, SendGrid key, Telegram token)
  live in the host's gitignored .env; .env.example documents every key without leaking secrets.
  - Branch protection on UNagent-1D/dev-runner requires one approving review, both CI checks (compose
  validation + Compliance ≥85 % test coverage) green, no force-push, and linear history before merging
   into main.
...
> **\[Diagram TBD]** — to be exported from the team's draw.io / Lucid
> source as `docs/img/cnc.png`.
...
is the only component exposed to web clients; every other backend
service is reachable only through it, through the Agent Runtime, or
(for analytics reads and admin login) through `chat-orch` and
`Tenant` directly.
...
`Hospital-MP`, `Compliance`, `UN_email_send_ms`, `FrontEnd`), each with
...
email-mongo, Conversation cache → Redis). No two services share a
schema; cross-service queries always go through HTTP.

#### 3.1.3. Architectural Elements

The system exposes **two presentation** components, **seven logic**
components, **one orchestration/messaging** component, and **five
...
| `conversation-chat` — Conversation Service | Go + Gin + mongo-driver + go-redis + AMQP (`amqp091-go`) | Durable session and per-turn history. Streams the OpenRouter response. Async worker drains `chat_requests` / produces `chat_results`. |
...
| `Compliance` — KPI + Audit Service *(new)* | Python 3.12 + FastAPI + motor (async Mongo) | In-memory KPI counters and daily buckets per tenant. Back-compatible with the Metricas wire contract (`/conversation/chat`, `/feedback/csat`, `/stats/kpis`, `/stats/timeseries`) plus new `/v1/event` and `/v1/feedback` audit endpoints persisted to Mongo. |
| `Hospital-MP` — Hospital Mock API | Python 3.12 + Flask + psycopg2 | Postgres-backed scheduling domain. Five HTTP operations consumed by the Agent Runtime tool loop. |
| `UN_email_send_ms` — Email Service *(new)* | Java 21 + Spring Boot 3.3 + Spring Data MongoDB + sendgrid-java + jjwt | Outbound transactional email via SendGrid (sandbox-mode by default in dev). Persists every attempt to `email-mongo` (`email_events` collection). |

**Communication / Orchestration component**

| Component | Stack | Responsibility |
|---|---|---|
| `UN_message_broker_mb` — Message Broker *(new)* | RabbitMQ 3-management + `definitions.json` declarative bootstrap | Hosts the `chat_requests` and `chat_results` durable queues. Plays the **publish/subscribe connector** role; consumed by `agent-runtime` and `conversation-chat::worker`. |

**Data components (five total — relational + NoSQL covered with margin)**
...
| `hospital-postgres` *(new)* | Relational | Local container, `hospital-postgres-data` volume | `Hospital-MP` | `doctors` and `appointments` tables. Schema + seed applied on first boot via `/docker-entrypoint-initdb.d`. |
| `email-mongo` *(new)* | NoSQL (document) | Local container, `email-mongo-data` volume | `email-send`, `Compliance` | Two databases on one Mongo instance: `email_audit.email_events` and `UN_compliance_db.audit_logs`. |
| Redis | NoSQL (key-value, cache) | Local container | `conversation-chat` | Active session cache. |
...
| 4 | **AMQP 0.9.1 (Pub/Sub) — *new*** | TCP, RabbitMQ wire protocol | `agent-runtime` ↔ `chat_requests` / `chat_results` ↔ `conversation-chat::worker`. |
| 5 | **PostgreSQL wire protocol** | TCP (Supabase Session Pooler / local) | `Tenant ↔ Supabase`; `Hospital-MP ↔ hospital-postgres`. |
| 6 | **MongoDB wire protocol** | TCP (Atlas SRV / local) | `conversation-chat ↔ Atlas`; `email-send` and `Compliance` ↔ `email-mongo`. |
| 7 | **Redis RESP** | TCP | `conversation-chat ↔ redis`. |
...
*Devices, Cloudflare edge, and external SaaS.*

![UNAgent — Deployment context](deployment-1-context.svg)

*Source: `docs/deployment-1-context.puml`.*
...
*Source: `docs/deployment-2-host.puml`.*
...
4. **Host** — a Linux box (developer laptop or Oracle Cloud A1.Flex
   Always-Free VM) running Docker engine 29 + containerd 2.2.3 +
   runc 1.4. The Docker engine is itself an `«executionEnvironment»`
   that hosts a bridge network `archsoft`, which in turn hosts the
   12 service containers and 2 named volumes.

**Containers (twelve `«executionEnvironment»` nodes inside `archsoft`).**
...
- **Intra-host** (between components in the bridge network, all over
  ...
  - AMQP 0.9.1 between `agent-runtime`, `conversation-chat::worker`,
    and `rabbitmq`.
  - TCP for Postgres, Mongo, and Redis client connections.
...
#### 3.2.3. Deployment Patterns Used
...
**2. Sidecar-less reverse proxy / Edge tunnel (Cloudflare Tunnel).**
...
**5. Healthcheck-gated start order.** All databases
(`hospital-postgres`, `email-mongo`, `redis`, `rabbitmq`) and the
`tenant` service expose Docker `healthcheck` directives; every
dependent service uses `depends_on` with `condition:
service_healthy`, which Docker Compose enforces during `up`. This
avoids the "service started but Postgres not ready" race.

**6. Build-time configuration baking (FrontEnd).** Vite (and Next.js
when statically exporting) inlines `VITE_*` / `NEXT_PUBLIC_*`
environment variables at build time. The umbrella compose passes
them as `build.args`, so the FrontEnd image is per-environment but
runtime-immutable — perfect for Cloudflare Pages, where the bundle
is uploaded once and served from the CDN.

**7. Trial / Always-Free hybrid.** Stateful infrastructure containers
(`hospital-postgres`, `email-mongo`, `redis`, `rabbitmq`) live on the
Always-Free Oracle VM; large-scale databases (Atlas, Supabase) and
the LLM and email providers (OpenRouter, SendGrid) run on their
respective managed free tiers. None of the production deployment
incurs cost.
...
> **\[Diagram TBD]** — to be exported as `docs/img/layered.png`.

![Un Agent — Layered view](img/layered.png)

#### 3.3.2. Architectural Elements and Relations

The platform is organised in **six layers**. Dependencies flow
strictly downward — a layer may call any layer below it but never
above.

| # | Layer | Members |
|---|---|---|
| 1 | **Presentation** | `FrontEnd` (Next.js SSR), Telegram channel (in-process within `chat-orch`) |
| 2 | **Orchestration** | `chat-orch` (front-door + LLM turn loop) |
| 3 | **Application logic** | `tenant`, `agent-runtime`, `conversation-chat`, `compliance`, `hospital-mock`, `email-send` |
| 4 | **Integration / Messaging** | `RabbitMQ` broker, `agent-runtime::broker` and `conversation-chat::worker` AMQP clients |
| 5 | **Data** | Supabase Postgres, MongoDB Atlas, `hospital-postgres`, `email-mongo`, Redis |
| 6 | **External providers** | OpenRouter, SendGrid, Telegram Bot API |

**Allowed dependencies (downward only).**

- L1 → L2 — the FrontEnd and Telegram channel call the orchestrator.
- L1 → L3 — the FrontEnd also calls `tenant` (login) and `compliance`
  (KPIs) directly to avoid round-tripping through the orchestrator
  for non-chat traffic.
- L2 → L3 — `chat-orch` fans out to most logic services.
- L3 → L3 — `agent-runtime` mediates `conversation-chat` and
  `hospital-mock` (sibling collaboration only via `agent-runtime`).
- L3 → L4 — `agent-runtime` and `conversation-chat::worker` produce
  and consume on RabbitMQ.
- L3 → L5 — every logic service uses its own data store.
- L2 / L3 → L6 — `chat-orch` calls OpenRouter and Telegram;
  `email-send` calls SendGrid; `conversation-chat` also calls
  OpenRouter for tool execution.

No upward call exists anywhere — including no calls from the data
layer back into application logic. This is **strict layering** for
upward direction and **relaxed layering** downward (since L1 can skip
L2 to reach L3).

#### 3.3.3. Architectural Patterns Used

**1. Strict-on-up, relaxed-on-down layered architecture.** Allowed
dependencies are encoded in compose via `depends_on` and in code via
the env-var URL conventions (every service knows only the layer
below it).

**2. Inversion of intermediaries via Backend-For-Frontend.** L1
"shortcuts" L2 only for read-only admin paths (KPIs, login) where
the orchestrator would add no value. All chat-path traffic still
goes L1 → L2 → L3.

**3. Anti-corruption layer between Agent Orchestrator and Conversation.**
`agent-runtime` adapts the thin `OpenSessionRequest` /
`TurnRequest` payloads from `chat-orch` into the richer payloads
expected by `conversation-chat`. This isolates the two services from
...
> **\[Diagram TBD]** — to be exported as `docs/img/decomposition.png`.

![Un Agent — Decomposition view](img/decomposition.png)
...
**7. Hospital Subsystem** (`Hospital-MP`) — modules: Flask endpoints,
DB layer (`db.py`, psycopg2), schema + seed scripts.
...
**10. Data Subsystem** — five owned databases (Supabase, Atlas,
`hospital-postgres`, `email-mongo`, Redis). Each is bound to exactly
one application-tier subsystem.
...
| `Hospital-MP/` | `UNagent-1D/Hospital-MP` | Python | Mock hospital scheduling API (Postgres-backed) |
...
### 4.7. Production deployment (Oracle Cloud + Cloudflare)

The umbrella ships two helper scripts and a deployment runbook:

| Path | Purpose |
|---|---|
| `scripts/oci-bootstrap.sh` | Idempotent provisioner (OCI CLI) — VCN + subnet + IGW + route + A1.Flex VM with public IP. |
| `scripts/oci-launch-retry.sh` | Wrapper that retries on `Out of host capacity` and auto-downgrades shape if needed. |
| `AGENTS.md` §11.1 | Step-by-step Oracle VM + Cloudflare Pages + Cloudflare Tunnel runbook. |

After the VM is up, the deployment is a `git clone --recursive` plus
`docker compose up -d` + a `cloudflared` tunnel pointing at the
backend ports.
...
| **≥ 5 logic components** | `chat-orch`, `tenant`, `agent-runtime`, `conversation-chat`, `Hospital-MP`, `Compliance`, `email-send` — **7 total**. |
| **≥ 1 communication / orchestration component** | `chat-orch` (synchronous orchestration) + `RabbitMQ` (asynchronous Pub/Sub broker). |
| **≥ 4 data components (relational + NoSQL)** | Supabase Postgres + MongoDB Atlas + `hospital-postgres` + `email-mongo` + `Redis` — **5 total** (2 relational, 3 NoSQL: 2× document + 1× key-value cache). |
...
| **Container-oriented deployment** | Every service ships a `Dockerfile`; the full stack is orchestrated via Docker Compose from `dev-runner`. The production target is a single Oracle Cloud A1.Flex VM running the same compose stack, fronted by Cloudflare Pages + Tunnel. |

---

*Generated 2026-05-04 alongside the diagrams in `docs/`.*
